<div class="page-stack form-page">
    <a class="back-link" href="{{ route('patients.show', ['grantId' => $grantId]) }}" wire:navigate><span aria-hidden="true">←</span> {{ data_get($grant, 'pet.name', 'Patient') }}</a>
    ...
    @else
        <form class="clinical-form" wire:submit="submit" novalidate>
            ...
            <footer class="form-footer">
                ...
                <div class="button-row"><a class="secondary-button" href="{{ route('patients.show', ['grantId' => $grantId]) }}" wire:navigate>Cancel</a><button class="primary-button" type="submit" wire:loading.attr="disabled" wire:target="submit"><span wire:loading.remove wire:target="submit">Send for family review</span><span wire:loading wire:target="submit">Sending securely…</span></button></div>
            </footer>
        </form>
    @endif
</div>
