...
class PortalOAuthController
{
    ...
    public function redirect(Request $request): RedirectResponse
    {
        ...
        abort_unless(is_string($clientId) && $clientId !== '' && is_string($clientSecret) && strlen($clientSecret) >= 32 && is_string($redirectUri) && filter_var($redirectUri, FILTER_VALIDATE_URL) && filter_var($canonicalPortalUrl, FILTER_VALIDATE_URL) && PlatformConfiguration::isSafe(), 503, 'This portal has not been connected to Zigpaw yet.');
    ...
    {
        abort_unless(PlatformConfiguration::isSafe(), 503, 'This portal has not been connected to Zigpaw yet.');
    ...
    public function logout(Request $request, PortalAccessTokenStore $tokens): RedirectResponse
    {
        ...
        if ($logoutUrl === null) {
            return redirect()->route('dashboard')->with('success', 'You are signed out on this device.');
        }
