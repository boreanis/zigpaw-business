...
trait UsesPortalWorkspace
{
    ...
    protected function portalCredentials(PortalAccessTokenStore $tokens): ?array
    {
    ...
    protected function clearPortalSessionIfUnauthorized(int $status, PortalAccessTokenStore $tokens): void
    {
        if ($status === 401) {
            $tokens->forget();
