...
class CareSubmission extends Component
{
    ...
    public function submit(PlatformApiClient $api, ClinicalPortalAccessTokenStore $tokens): void
    {
        if (! $this->canSubmit) {
        ...
        try {
            $submission = $api->createClinicalCareSubmission(
                $tokens,
                $organizationId,
                $this->grantId,
            ...
            $this->redirectRoute('clinical.submissions.show', ['submissionId' => $submission['id']], navigate: true);
    ...
    private function loadFormContext(PlatformApiClient $api, ClinicalPortalAccessTokenStore $tokens): void
    {
        $credentials = $this->portalCredentials($tokens);
        if ($credentials === null) {
            return;
        ...
        try {
            ...
            $identity = $api->clinicalIdentity($tokens, $organizationId);
            $dashboard = $api->clinicalDashboard($tokens, $organizationId);
