...
class PatientShow extends Component
{
    ...
    private function loadPatient(PlatformApiClient $api, PortalAccessTokenStore $tokens): void
    {
        ...
        try {
            ...
            if (($capabilities['read_care'] ?? false) === true) {
                $this->care = $api->careContext($accessToken, $organizationId, $this->grantId);
            }

            if (($capabilities['read_media'] ?? false) === true) {
                $this->media = $api->providerMedia($accessToken, $organizationId, $this->grantId);
            }
